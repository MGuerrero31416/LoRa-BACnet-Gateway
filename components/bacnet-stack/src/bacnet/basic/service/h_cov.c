...
/** Handler for a SubscribeCOVProperty Service request.
 * @ingroup DSCOV
 * Supports Present_Value subscriptions by mapping to standard COV.
 */
    ...
    BACNET_CONFIRMED_SERVICE_DATA *service_data)
{
    ...
    if (service_len == 0) {
        ...
        if (len < 0) {
            ...
            /* Only Present_Value is supported for property subscriptions */
            if ((cov_data.monitoredProperty.property_identifier !=
                    PROP_PRESENT_VALUE) ||
                (cov_data.monitoredProperty.property_array_index !=
                    BACNET_ARRAY_ALL)) {
                cov_data.error_class = ERROR_CLASS_PROPERTY;
                cov_data.error_code = ERROR_CODE_UNKNOWN_PROPERTY;
                error = true;
            } else {
                cov_data.error_class = ERROR_CLASS_OBJECT;
                cov_data.error_code = ERROR_CODE_UNKNOWN_OBJECT;
                success = cov_subscribe(
                    src, &cov_data, &cov_data.error_class,
                    &cov_data.error_code);
                if (success) {
                    apdu_len = encode_simple_ack(
    ...
    if (error) {
        if (len == BACNET_STATUS_ABORT) {
            apdu_len = abort_encode_apdu(
                &Handler_Transmit_Buffer[npdu_len],
                service_data->invoke_id, cov_data.error_code, true);
        } else if (len == BACNET_STATUS_REJECT) {
            apdu_len = reject_encode_apdu(
                &Handler_Transmit_Buffer[npdu_len],
                service_data->invoke_id, cov_data.error_code);
        } else {
            apdu_len = bacerror_encode_apdu(
                &Handler_Transmit_Buffer[npdu_len],
                service_data->invoke_id,
    ...
    bytes_sent = datalink_send_pdu(
        src, &npdu_data, &Handler_Transmit_Buffer[0], pdu_len);
    if (bytes_sent <= 0) {
        debug_perror("SubscribeCOVProperty: Failed to send PDU");
    }
}
